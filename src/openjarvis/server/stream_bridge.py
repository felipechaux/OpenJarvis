...
class AgentStreamBridge:
    ...
    async def stream(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # IMPORTANT: do not re-stream by re-querying ``engine.stream_full``
            # with the raw client messages.  That path discards the agent's
            # system prompt, tool calls, and observations — Gemini in particular
            # falls back to its trained ``tool_code`` protocol and leaks
            # ``TOOL CALL`` / ``## TOOL RESPONSE`` blocks into the chat.
            # The agent's already-vetted ``agent_result.content`` is the only
            # text the user should ever see, so we always replay it.
